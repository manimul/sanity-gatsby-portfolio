...
function Hero (props) {
 ...
 <div class="flex flex-col w-full md:w-1/2 justify-center items-start text-center md:text-left">
     <p class="uppercase  text-center md:text-left tracking-loose w-full">{props.subtitle}</p>
     <h1 class="my-4  w-full text-center md:text-left text-3xl md:text-6xl font-serif leading-none">{props.title}</h1>
     <p class="leading-normal md:w-3/4 md:text-left md:text-2xl md:mb-8">{props.description}</p>
